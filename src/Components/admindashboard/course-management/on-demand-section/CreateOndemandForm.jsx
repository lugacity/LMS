...
import {
  ...
  useEditOnDemandCourse,
} from "@/hooks/course-management/on-demand/use-create-demand-course";
...
function CreateOndemandForm({ sectionToEdit = {}, setEdit }) {
  ...
  const { editOnDemandCourse, isEditing } = useEditOnDemandCourse();

  const { id, ...values } = sectionToEdit;
  ...
  const form = useForm({
    ...
    defaultValues: isEdit
      ? {
          title: values.title,
          video_title: values.lessons[0].video_title,
          overview: values.overview,
          video_from_url: "https://medusa.dev",
        }
      : {
          title: "",
          video_title: "",
  ...
  const createDemandSection = async (data) => {
    ...
    <Form {...form}>
      <form
        onSubmit={form.handleSubmit(
          isEdit ? editDemandSection : createDemandSection,
        )}
      >
        <div>
          <FormInput
        ...
        <div className="space-y-2">
          ...
          >
            ...
            <input
              ...
              disabled={form.watch("video_from_url").length >= 1 || isEdit}
            />
          </div>
          {errorMessage && (
        ...
        <div className="flex flex-col gap-y-4">
          <FormInput
            ...
            disabled={video.file || isEdit ? true : false}
          />
        </div>
        ...
        <div>
          <div className="ml-auto mt-6 w-max">
            ...
            >
              {isEdit ? "Edit section" : "Add Content"}
            </CommonButton>
          </div>
        </div>
      </form>
